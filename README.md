...
All Cognee databases and generated state are kept under `.cognee_data/`. The
store contains no model-specific ownership: every current or future model sees
the same chunks, graph, datasets, and provenance.
...
Retrieve context for any model without asking Cognee's extraction model to
write the answer:

```bash
curl -s http://127.0.0.1:8000/retrieve \
  -H 'Content-Type: application/json' \
  -d '{"query":"What does the gradient model use?","datasets":["smoke_test"]}'
```

Endpoints:

- `POST /remember`: ingest into the shared memory (uses the configured
  extraction model while building the graph).
- `POST /retrieve`: return relevant source chunks for any downstream model.
- `POST /answer`: optional answer generated by Cognee's configured LLM.
...
Changing or adding an Ollama model does not require re-ingestion. New models
consume the same `/retrieve` results.
